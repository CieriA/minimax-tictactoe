...
impl Board {
    ...
    fn check_cols(&self, last_coord: Point) -> bool {
        let b = self[last_coord].unwrap();
        self.iter()
            .all(|row| row[last_coord.x].is_some_and(|square| square == b))
    }
    ...
    fn check_diagonals(&self, last_coord: Point) -> bool {
        ...
        if x == y {
            (0..Board::SIZE).all(|i| self[Point::new(i, i)].is_some_and(|square| square == b))
        } else {
            [
                self[last_coord],
                self.center(),
                self[Point::new(last_coord.y, last_coord.x)],
            ]
            .into_iter()
            .all(|square| square.is_some_and(|square| square == b))
        }
    ...
    pub(crate) fn check_win(&self, last_coord: Point) -> bool {
        let in_diagonal =
            last_coord.x == last_coord.y || (last_coord.x + last_coord.y + 1) % Self::SIZE == 0;

        self.check_rows(last_coord)
            || self.check_cols(last_coord)
            || (in_diagonal && self.check_diagonals(last_coord))
    }
    #[inline]
    ...
    pub(crate) fn possible_moves(&self) -> Vec<Point> {
        self.iter()
            ...
            .flat_map(|(y, row)| {
                row.iter()
                    .enumerate()
                    .map(move |(x, square)| (Point::new(x, y), square))
            })
